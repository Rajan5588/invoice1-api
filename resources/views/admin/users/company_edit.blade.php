            ...
            <form action="{{ route('company.update', $company->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="row g-3 mt-1">
                    ...
                    <div class="col-md-6">
                        ...
                        @if($company->avatar)
                            <img src="{{ asset('assets/'.$company->avatar) }}" class="img-thumbnail mt-2" width="80">
                        @endif
                ...
                <div class="row g-3 mt-1">
                    <div class="col-md-12">
                        ...
                        <textarea name="full_address" rows="2" class="form-control @error('full_address') is-invalid @enderror">{{ old('full_address', $company->full_address) }}</textarea>
                        @error('full_address') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    </div>
                    <div class="col-md-3">
                        <label class="form-label">State</label>
                        <input type="text" name="state" value="{{ old('state', $company->state) }}" class="form-control @error('state') is-invalid @enderror">
                        @error('state') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    </div>
                    <div class="col-md-3">
                        <label class="form-label">District</label>
                        <input type="text" name="district" value="{{ old('district', $company->district) }}" class="form-control @error('district') is-invalid @enderror">
                        @error('district') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    </div>
                    <div class="col-md-3">
                        <label class="form-label">GST Number</label>
                        <input type="text" name="gst_no" value="{{ old('gst_no', $company->gst_no) }}" class="form-control @error('gst_no') is-invalid @enderror">
                    ...
                    <div class="col-md-3">
                        ...
                        <select name="role_id" class="form-control select2 @error('role_id') is-invalid @enderror">
                            ...
                            @foreach($roles as $role)
                                <option value="{{ $role->id }}" {{ old('role_id', $company->role_id) == $role->id ? 'selected' : '' }}>
                                    {{ $role->role_name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="subs_id" class="form-control select2 @error('subs_id') is-invalid @enderror">
                            ...
                            @foreach($subscriptions as $subscription)
                                <option value="{{ $subscription->id }}" {{ old('subs_id', $company->subs_id) == $subscription->id ? 'selected' : '' }}>
