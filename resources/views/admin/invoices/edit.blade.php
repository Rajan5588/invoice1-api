...
<div class="row justify-content-center">
    <div class="col-lg-12">
        <div class="card">
            <div class="card-header"><h6>Edit GST Invoice #{{ $invoice->id }}</h6></div>
            <div class="card-body">
                <form id="editInvoiceForm">
                    ...
                    <div class="row g-3 mb-3">
                        <div class="col-md-3">
                            ...
                            <input type="text" name="receiver_name" class="form-control" required
                                   value="{{ old('receiver_name', $invoice->billingDetail->name ?? '') }}">
                        </div>
                        <div class="col-md-3">
                            <label>Contact No.</label>
                            <input type="text" name="receiver_phone" class="form-control"
                                   value="{{ old('receiver_phone', $invoice->billingDetail->phone ?? '') }}">
                        </div>
                        <div class="col-md-3">
                            <label>Email</label>
                            <input type="email" name="receiver_email" class="form-control"
                                   value="{{ old('receiver_email', $invoice->billingDetail->email ?? '') }}">
                        </div>
                        <div class="col-md-3">
                            <label>State</label>
                            <select name="receiver_state_code" class="form-control state-change" required>
                                ...
                                @foreach($states as $code => $name)
                                    <option value="{{ $code }}" {{ (old('receiver_state_code', $invoice->billingDetail->state ?? '') == $code) ? 'selected' : '' }}>{{ $name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="col-md-4">
                            ...
                            <select id="consigneeCustomer" class="form-control">
                                ...
                                <option value="{{ $customer->id }}" 
                                    ...
                                    data-address="{{ $customer->place_of_supply }}"
                        ...
                        <div class="col-md-4">
                            ...
                            <select name="payment_type" class="form-control" required>
                                ...
                                @foreach($paymentTypes as $type)
                                    <option value="{{ $type }}" {{ (old('payment_type',$invoice->payment_type)==$type)?'selected':'' }}>{{ ucfirst(str_replace('_',' ',$type)) }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="row g-3 mb-3">
                        <div class="col-md-3"><label>Name</label>
                            <input type="text" name="consignee_name" id="consignee_name" class="form-control" required value="{{ old('consignee_name', $invoice->shippingDetail->name ?? '') }}">
                        </div>
                        <div class="col-md-3"><label>Contact</label>
                            <input type="text" name="consignee_phone" id="consignee_phone" class="form-control" value="{{ old('consignee_phone', $invoice->shippingDetail->phone ?? '') }}">
                        </div>
                        <div class="col-md-3"><label>Email</label>
                            <input type="email" name="consignee_email" id="consignee_email" class="form-control" value="{{ old('consignee_email', $invoice->shippingDetail->email ?? '') }}">
                        </div>
                        <div class="col-md-3"><label>State</label>
                            <select name="consignee_state_code" id="consignee_state_code" class="form-control state-change" required>
                                @foreach($states as $code=>$name)
                                    <option value="{{ $code }}" {{ old('consignee_state_code',$invoice->shippingDetail->state ?? '')==$code?'selected':'' }}>{{ $name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-6"><label>Address</label>
                            <textarea name="consignee_address" id="consignee_address" class="form-control">{{ old('consignee_address', $invoice->shippingDetail->address ?? '') }}</textarea>
                        </div>
                    </div>
                    ...
                    <table class="table table-bordered" id="itemsTable">
                        ...
                        @foreach($invoice->items as $index => $item)
                            <tr>
                                    ...
                                    <select name="items[{{ $index }}][item_id]" class="form-control item-select">
                                        ...
                                        @foreach($items as $it)
                                            <option value="{{ $it->id }}"
                                                ...
                                                data-sales="{{ $it->pricings->first()->salesprice_amount ?? 0 }}"
                                                data-sales-tax="{{ $it->pricings->first()->salesprice_tax ?? 0 }}"
                                                data-purchase="{{ $it->pricings->first()->purches_price_amount ?? 0 }}"
                                                data-purchase-tax="{{ $it->pricings->first()->purches_price_tax ?? 0 }}"
                                                data-mrp="{{ $it->pricings->first()->mrp_price ?? 0 }}"
                                                data-gst="{{ $it->pricings->first()->gst ?? 0 }}">
                                                {{ $it->item_name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <td>
                                    <select name="items[{{ $index }}][price_type]" class="form-control price-type">
                                        <option value="sales" {{ $item->price_type=='sales'?'selected':'' }}>Sales</option>
                                        <option value="purchase" {{ $item->price_type=='purchase'?'selected':'' }}>Purchase</option>
                                        <option value="mrp" {{ $item->price_type=='mrp'?'selected':'' }}>MRP</option>
                                    </select>
                                </td>
                                <td><input type="number" name="items[{{ $index }}][quantity]" class="form-control qty" value="{{ $item->quantity }}"></td>
                                <td><input type="number" name="items[{{ $index }}][price]" class="form-control price" step="0.01" value="{{ $item->price }}"></td>
                                <td><input type="number" name="items[{{ $index }}][gst]" class="form-control gst" readonly value="{{ $item->gst }}"></td>
                                <td><input type="number" class="form-control cgst" readonly value="{{ $item->cgst }}"></td>
                                <td><input type="number" class="form-control sgst" readonly value="{{ $item->sgst }}"></td>
                                <td><input type="number" class="form-control igst" readonly value="{{ $item->igst }}"></td>
                    ...
                    <hr>
                    <div class="row g-3">
                        <div class="col-md-3"><label>Discount %</label><input type="number" name="discount_percent" class="form-control" value="{{ $invoice->discount_percent }}"></div>
                        <div class="col-md-3"><label>Discount Amount</label><input type="number" name="discount_amount" class="form-control" value="{{ $invoice->discount_amount }}"></div>
...
$(function(){
    ...
    $('#editInvoiceForm').submit(function(e){
        ...
        $.ajax({
            ...
            method:'POST',
            data: $(this).serialize(),
            success:function(){ alert('Invoice updated'); window.location='{{ route("invoices.index") }}'; },
