...
<body>
  ...
  <section id="bill2" class="sheet">
    ...
    <div class="header">
      <div class="brand">
        <img src="{{ $businessProfile->user && $businessProfile->user->avatar
      ...
      <div class="invoice-head">
        <div class="title">{{ $businessProfile->business_name ?? 'Company Name' }}</div>
        <div class="copy">{{ $businessProfile->business_address ?? '' }}</div>
      </div>

      <div class="company">
        <div><strong>{{ $businessProfile->business_address ?? '' }}</strong></div>
        <div>{{ $businessProfile->city ?? '' }} {{ $businessProfile->pincode ?? '' }}</div>
        <div>{{ $businessProfile->website ?? '' }}</div>
        <div>Call us: {{ $businessProfile->phone_no_first ?? '-' }}</div>
        <div>Email: {{ $businessProfile->business_email ?? $businessProfile->email ?? '-' }}</div>
      </div>
    </div>
    ...
    <div class="lower">
      <div>
        <div class="block">
          ...
          <div class="body dl">
            <div class="k">Contact No. :</div><div class="v">{{ $invoice->billingDetail->phone ?? '-' }}</div>
            <div class="k">Name :</div><div class="v">{{ $invoice->billingDetail->name ?? '-' }}</div>
            <div class="k">E-mail id :</div><div class="v">{{ $invoice->billingDetail->email ?? '-' }}</div>
            <div class="k">State Code :</div><div class="v">{{ $invoice->billingDetail->state ?? '-' }}</div>
            <div class="k">GST No. :</div><div class="v">{{ $invoice->billingDetail->gst_number ?? '-' }}</div>
            <div class="k">PAN No. :</div><div class="v">{{ $invoice->billingDetail->pan_number ?? '-' }}</div>
          </div>
        </div>
        ...
        <div class="block">
          ...
          <div class="body dl">
            <div class="k">Contact No. :</div><div class="v">{{ $invoice->shippingDetail->phone ?? '-' }}</div>
            <div class="k">Name :</div><div class="v">{{ $invoice->shippingDetail->name ?? '-' }}</div>
            <div class="k">Delivery Address :</div><div class="v">{!! nl2br(e($invoice->shippingDetail->address ?? '-')) !!}</div>
            <div class="k">E-mail id :</div><div class="v">{{ $invoice->shippingDetail->email ?? '-' }}</div>
          </div>
        </div>
        ...
        <div class="block">
          ...
          <div class="body dl">
            <div class="k">Awarded Points:</div><div class="v">{{ $invoice->loyalty_points ?? 0 }}</div>
            <div class="k">Customer's Wallet :</div><div class="v">{{ $invoice->customer_wallet ?? 0 }}</div>
            <div class="k">Current Invoice Loyalty Expiry Date</div>
            <div class="v">{{ optional($invoice->loyalty_expiry_date)->format('d F Y') ?? '-' }}</div>
          </div>
        </div>
      </div>
      ...
      <div>
        <div class="meta">
          <div class="left dl">
            <div class="k">Invoice No. :</div><div class="v">invoice#{{ $invoice->invoice_number ?? $invoice->id }}</div>
            <div class="k">Invoice Date :</div><div class="v">{{ $invoice->created_at->format('d M Y') ?? '-' }}</div>
            <div class="k">GST No. :</div><div class="v">{{ $businessProfile->gst_no ?? '-' }}</div>
            <div class="k">PAN No. :</div><div class="v">{{ $businessProfile->pan_number ?? '-' }}</div>
            <div class="k">CIN No. :</div><div class="v">{{ $businessProfile->cin_number ?? '' }}</div>
            <div class="k">E-mail id :</div><div class="v">{{ $businessProfile->business_email ?? $businessProfile->email ?? '-' }}</div>
        ...
        <div class="block">
          <div class="head">—</div>
          <div class="body dl">
            <div class="k">Salesperson Name :</div><div class="v">{{ $invoice->user->sname ?? '-' }}</div>
            <div class="k">Payment Mode :</div><div class="v">{{ ucfirst($invoice->payment_type ?? '-') }} - {{ number_format($invoice->amount_received ?? 0, 2) }}</div>
            <div class="k">Buyer's PO No. :</div><div class="v">{{ $invoice->po_number ?? '-' }}</div>
            <div class="k">EMI Option :</div><div class="v">{{ $invoice->emi_option ?? '-' }}</div>
            <div class="k">Approval code :</div><div class="v">{{ $invoice->approval_code ?? '-' }}</div>
            <div class="k">Delivery Address :</div><div class="v">{!! nl2br(e($invoice->billingDetail->address ?? '-')) !!}</div>
            <div class="k">Buy Back Amt + Service Code :</div><div class="v">{{ $invoice->buyback_amount ?? 0 }}</div>
            <div class="k">Gift Voucher Awarded :</div><div class="v">{{ $invoice->gift_voucher ?? '-' }}</div>
            <div class="k">Reference :</div><div class="v">{{ $invoice->reference ?? '-' }}</div>
            <div class="k">BB serial no. :</div><div class="v">{{ $invoice->bb_serial ?? '-' }}</div>
          </div>
        </div>
      </div>
    </div>
      ...
      <tbody>
        ...
        @foreach($invoice->gstDetails as $gst)
          ...
          <tr>
            <td>
              {{ $gst->item->sku ?? '-' }}
              <b>{{ $invoiceItem->item->item_name ?? '-' }} </b><br>
              {{ $invoiceItem->item->details->item_description ?? '-' }}
            </td>
            <td>{{ number_format($gst->price, 2) }}</td>
            <td>{{ $invoice->discount_percent ?? 0 }}</td>
            <td>{{ $gst->item->hsn_sac ?? '-' }}</td>
            <td>{{ $gst->quantity }}</td>
            <td>{{ number_format($taxable, 2) }}</td>

            {{-- CGST --}}
            <td>{{ $cgst > 0 ? ($gst->gst_percent/2).'%' : '0%' }}</td>
            <td>{{ number_format($cgst,2) }}</td>

            {{-- SGST --}}
            <td>{{ $sgst > 0 ? ($gst->gst_percent/2).'%' : '0%' }}</td>
        ...
        <tr class="bold">
          ...
          <td></td><td>{{ number_format($totalCGST,2) }}</td>
          <td></td><td>{{ number_format($totalSGST,2) }}</td>
          <td></td><td>{{ number_format($totalIGST,2) }}</td>
          <td>{{ number_format($invoice->total_amount ?? $grandTotal,2) }}</td>
        </tr>
      </tbody>
    </table>
    ...
    <div class="footnote">
      ...
      {!! nl2br(e($invoice->note ?? 'Standard terms apply.')) !!}
    </div>

    <!-- Signature -->
    <div class="sign">
      {{ $businessProfile->business_name ?? 'Company Name' }}<br>
      <img src="{{ asset($businessProfile->business_signature ?? '') }}" alt=""><br>
